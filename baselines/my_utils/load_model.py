...
from transformers.models.qwen2.modeling_qwen2 import QWEN2_ATTENTION_CLASSES, Qwen2ForCausalLM
...
Qwen2ForCausalLM.forward = my_forward
...
def load_model(model_id, modified=None, torch_dtype=torch.float16, device_map='auto', flash_attention_2=False):
    ...
    if modified == 'gemfilter':
        ...
        from my_baseline.GemFilter.qwen2_select_attention import Qwen2SelectAttention
        ...
        QWEN2_ATTENTION_CLASSES[attn_implementation] = Qwen2SelectAttention
    elif modified == 'snapkv':
        assert flash_attention_2 is True
        from my_baseline.SnapKV.monkeypatch import replace_llama, replace_mistral, replace_phi3, replace_qwen
        replace_llama()
        replace_mistral()
        replace_phi3()
        replace_qwen()
        ...
        from my_baseline.H2O.qwen2_h2o_attention import Qwen2H2OAttention
        ...
        QWEN2_ATTENTION_CLASSES[attn_implementation] = Qwen2H2OAttention
    else:
