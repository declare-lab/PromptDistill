...
from transformers.models.llama.modeling_llama import LlamaConfig,Cache,Union,List,Unpack,FlashAttentionKwargs,KwargsForCausalLM,LlamaRotaryEmbedding,LlamaMLP,LlamaRMSNorm,DynamicCache,StaticCache,apply_rotary_pos_emb,repeat_kv,logger,eager_attention_forward,repeat_kv
...
class SelectiveLlamaModel(LlamaPreTrainedModel):
...
class PromptDistill(LlamaPreTrainedModel, GenerationMixin):
    _tied_weights_keys = ["lm_head.weight"]
    _tp_plan = {"lm_head": "colwise_rep"}

    def __init__(self, config):
        super().__init__(config)
        self.model = SelectiveLlamaModel(config)
        self.vocab_size = config.vocab_size
